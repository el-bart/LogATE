...
private:
  ...
  std::unique_ptr<LogATE::Tree::Node> createFilterBasedOnSelection(DS const& ds, Curses::DataSource::Id const& id) const;
  template<typename DS>
  void inspectElement(DS const& ds, Curses::DataSource::Id const& id) const;
